      <div class="col-md-3 col-lg-2 p-3">
          <form method="GET" action="{{ url()->current() }}" class="mb-4 filter-form">
              <h5>Фильтры</h5>
              ...
              <div class="d-flex flex-column gap-2">
                  ...
                  <a href="{{ url()->current() }}" class="btn btn-danger" onclick="resetFilters()">Сбросить</a>
              </div>
              ...
              <script>
                  function resetFilters() {
                      ...
                      // Убираем все параметры фильтров из URL
                      currentUrl.searchParams.delete('category');
                      currentUrl.searchParams.delete('type');
                      currentUrl.searchParams.delete('brand');
                      currentUrl.searchParams.delete('power');
                      currentUrl.searchParams.delete('madein');
                      currentUrl.searchParams.delete('basetype');
                      currentUrl.searchParams.delete('price_from');
                      currentUrl.searchParams.delete('price_to');
                      currentUrl.searchParams.delete('in_stock');

                      // Переходим на новый URL без фильтров
                      window.location.href = currentUrl.toString();
                  }
              </script>

              {{-- Категория товара --}}
              @if (!empty($showCategoryFilter) && $showCategoryFilter)
              <div class="mb-3">
              ...
              {{-- Тип товара для бд --}}
              @if (!empty($showTypeFilterForBase) && $showTypeFilterForBase)
              <div class="mb-3">
              ...
              {{-- Тип товара --}}
              @if (!empty($showTypeFilter) && $showTypeFilter)
              <div class="mb-3">
                  <label for="type">Тип</label>
                  <select name="type" id="type" class="form-control">
                      <option value="">Все типы</option>
